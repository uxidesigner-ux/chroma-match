...
def ribbon(name, path, widths, flatten=0.35, tilt=0.0, resolution: int = 24):
    ...
    flat against the head rather than edge-on to it. A list of tilts, one per
    path point, lets the section roll along the lock.
    """
    ...
    if isinstance(tilt, (int, float)):
        tilts = [float(tilt)] * len(path)
    else:
        tilts = list(tilt)
        if len(tilts) < len(path):
            tilts = tilts + [tilts[-1]] * (len(path) - len(tilts))
